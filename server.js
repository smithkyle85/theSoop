...
function getComments(url, request) {
  ...
  response.body = {
    articles: Object.keys(database.comments)
        .map(commentId => database.comments[articleId])
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    //dont know why this didn't work.....
    /*savedComment.articleId.forEach(articleId => {
      const articleCommentId = database.articles[articleId].commentIds;
      //database.articles.commentIds[id] = null;
      articleCommentId.splice(articleCommentId.indexOf(id), 1);
    });*/
